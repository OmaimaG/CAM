...
const ReplayPage = () => {
  ...
  useEffect(() => {
    async function fetchTickets() {
      try {
        const response = await fetch('/api/v1/r');
  ...
  return (
    <div>
      <div>
        <p>Number of reclamation : {tickets.length}</p>
      </div>
      ...
      <tr key={index + 1}>
        <td className="px-6 py-3 bg-gray-50 text-left text-lg leading-4">{index + 1}</td>
                ...
                <Link className="px-4 py-2 rounded-full bg-red-600 ml-2 text-white "
                 href={`/chef-department/dashboard/reclamation/replay/${ticket.id}`}>
                 See Details
                </Link>
                <Link className="px-4 py-2 rounded-full bg-red-600 ml-2 text-white"
  href={`/chef-department/dashboard/reclamation/replay/update/${ticket.id}`}>
  Replay
</Link>
            ...
            <tr>
              <td colSpan="6">No Reclamation available</td>
            </tr>
          )}
        </tbody>
      </table>

...
const ReplayPage = () => {
  ...
  useEffect(() => {
    async function fetchTickets() {
      try {
        const response = await fetch('/api/v1/repaly');
      ...
      <tr key={index + 1}>
        <td className="px-6 py-3 bg-gray-50 text-left text-lg leading-4">{index + 1}</td>
                ...
                <Link className="px-4 py-2 rounded-full bg-red-600 ml-2 text-white "
                 href={`/chef-department/dashboard/ticket/replay/${ticket.id}`}>
                 See Details
                </Link>
                <Link className="px-4 py-2 rounded-full bg-red-600 ml-2 text-white"
  href={`/chef-department/dashboard/ticket/replay/update/${ticket.id}`}>
  Replay
</Link>
